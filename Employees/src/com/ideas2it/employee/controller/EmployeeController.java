...
public class EmployeeController extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		...
		case "/updateEmployee":
			...
			break;
		case "/getEmployeeId":
			getEmployeeId(request, response);
			break;
	...
	private void insertEmployee(HttpServletRequest request, HttpServletResponse response)
			throws IOException, ServletException {
		try {
			//Employee employee = employeeService.generateId();
			...
			employee.setStatus("Not Confirmed");
	...
	public void listEmployees(HttpServletRequest request,HttpServletResponse response) throws IOException, ServletException { 
         try{
	  ...
	  public void getEmployeeId(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
	    	String id = request.getParameter("eid");
	    	Integer employeeid = Integer.parseInt(id);
	    	Employee employeeToBeUpdated = employeeService.getEmployee(employeeid);
	    	updateId = employeeid;
	    	request.getSession().setAttribute("employeeToBeUpdated", employeeToBeUpdated);
	    	RequestDispatcher rd = request.getRequestDispatcher("UpdateEmployees.jsp"); 
		    rd.forward(request, response);
    	...
    	try { 
		  ...
		  public void getAssignEmployeeId(HttpServletRequest request,HttpServletResponse response) throws IOException, ServletException { 
			  ...
			  public void assignProject(HttpServletRequest request, HttpServletResponse response) { 
				  String id[] = request.getParameterValues("pro"); 
				  int projectid [] = new int[10];
				  int index = 0;
				  for(String proid : id) {
					  Integer projId = Integer.parseInt(proid);
					  projectid[index] = projId;
					  index++;
				  }
				  employeeService.assignProject(assignEmployeeId,projectid); 
			  }
